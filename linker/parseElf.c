...
static int read_elf(const char *filename, uint64_t bufaddr)
{
    ...
    while(fgets(line,MAX_ELF_FILE_WIDTH,fp) != NULL){
        ...
        //to this line,this line is not white and contains infomation
        ...
        if(line_counter < MAX_ELF_FILE_LENGTH){
            ...
            debug_printf(DEBUG_LINKER, "elf file %s is to long (>%d)\n", filename, MAX_ELF_FILE_LENGTH);
            exit(1);
        }
    }
...
void parse_elf(char *filename, elf_t *elf)
{
    ...
    elf->sht = malloc(elf->sht_count * sizeof(sh_entry_t)); // free
    ...
    elf->symt = malloc(elf->symt_count * sizeof(st_entry_t)); //free
